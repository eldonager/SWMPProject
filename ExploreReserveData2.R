...
nut <- datNut %>%
  dplyr::select("station", "year", "month", "chla_n", "nh4f")
...
d %>%
  filter(sta == "pdbbp" | sta == "pdbby") %>%
  ggplot(aes(x = YearFrac, y = nh4f, color = sta)) +
  geom_line() +
  scale_color_manual(values = c("pdbbp" = "blue", "pdbby" = "red"),
                     labels = c("Ploeg Channel", "Bayview Channel")) +
  labs(x = "Year Fraction", y = "Ammonium")

#### ammonium vs chl #####
d %>%
  filter(sta == "pdbbp" | sta == "pdbby") %>%
  filter(chla_n < 90)%>%
  ggplot(aes(x = nh4f, y = chla_n)) +
  geom_point() +
  geom_smooth(method = lm, se =F) +
  ylim(0,20)+
  facet_wrap(~sta, labeller = labeller(sta = c("pdbbp" = "Ploeg Channel", "pdbby" = "Bayview Channel"))) +
  labs(x = "Ammonium", y = "Chlorophyll")

#### ammonium vs turbidity ####
d %>%
  filter(sta == "pdbbp" | sta == "pdbby") %>%
  filter(chla_n < 90)%>%
  ggplot(aes(x = nh4f, y = turb_mean)) +
  geom_point() +
  geom_smooth(method = lm, se =F) +
  facet_wrap(~sta, labeller = labeller(sta = c("pdbbp" = "Ploeg Channel", "pdbby" = "Bayview Channel"))) +
  labs(x = "Ammonium", y = "Turbidity")
